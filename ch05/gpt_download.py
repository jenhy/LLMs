...
"""def download_and_load_gpt2(model_size, models_dir):
    ...
    # Kaggle Dataset 挂载目录（结构必须匹配）
    kaggle_dataset_path = f"/kaggle/input/gpt2-124m-model/{models_dir}/{model_size}"

    # 优先从 Kaggle Dataset 加载
    if os.path.exists(kaggle_dataset_path):
        print(f"使用 Kaggle Dataset 中的模型：{kaggle_dataset_path}")
        model_dir = kaggle_dataset_path
    else:
        print("未检测到 Kaggle Dataset,将尝试下载模型文件")
